...
using namespace ns3;
//NS_LOG_COMPONENT_DEFINE ("HiddenNodes");
/// Run single 10 seconds experiment
void experiment (bool enableCtsRts, std::string wifiManager,std::string myrate,int num_nodes)
{
  
  
  // Enable or disable CTS/RTS based on argument enableCtsRts
  //ctsThr is the frame size over which RTS/CTS will be applied
  // It is set to a low value when enableRtsCts is true (so that)
  // it's mostly applied, and set to a high value when enableRtsCts is fales
  //so that it's mostly applied
  
  //This statement sets the threshold variable
  UintegerValue ctsThr = (enableCtsRts ? UintegerValue (100) : UintegerValue (10000)); // <--------WHATTT??
  ...
  for (uint8_t i = 0; i <= num_nodes; ++i)
    {
      nodes.Get (i)->AggregateObject (CreateObject<ConstantPositionMobilityModel> ()); // <----Whattt?
    }
 ...
 //With this statement we are assuming that 10.0.0.1 as the IP address of node "n0"
  ...
  uint32_t payloadSize = 2200;                       /* Transport layer payload size in bytes. */      // <-----WHATTT??
  std::string dataRate = myrate; 
...
// flow 1:  node 1 -> node 0. Set start time attributes  
  ...
  for(uint8_t i=1;i<=num_nodes;i++){
   double stime;
   stime = 1.0000+  (double) i/(100.0+2*num_nodes);  
  ...
  for(uint8_t i=1;i<=num_nodes;i++){
    echoClientHelper.SetAttribute ("StartTime", TimeValue (Seconds ((double)i/1000)));
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
      // first 2 FlowIds are for ECHO apps, we don't want to display them   <------------Why??
      //
      // Duration for throughput measurement is 7.0 seconds, since
      //   StartTime of the OnOffApplication is at about "second 1"
      // and
      //   Simulator::Stops at "second 8".
      if (i->first > (uint8_t)(num_nodes))
        {
...
int main (int argc, char **argv)
{
  ...
  //std::string x [10] ={"5.4Mbps","10.8Mbps","16.2Mbps","21.6Mbps","27Mbps","32.4Mbps","37.8Mbps","43.2Mbps","48.6Mbps","54Mbps"};
  std::string x [1] = {"2Mbps"};
  //int i=2;
  for(int i=1;i<10;i++){
    ...
    std::cout << "Hidden station experiment with RTS/CTS disabled:for num_nodes = "<<i<<"\n"<< std::flush;
    experiment (false, wifiManager,x[0],i);
    std::cout << "------------------------------------------------\n";
    std::cout << "Hidden station experiment with RTS/CTS enabled: for num_nodes = "<<i<<"\n" ;
    experiment (true, wifiManager,x[0],i);
    //std::cout << "\n#########################################3\n";
  }
