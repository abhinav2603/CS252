...
void experiment (bool enableCtsRts, std::string wifiManager, int Mftp)
{
...
//Now Install this helper object and add the application to the cbrApps container
  	...
  	for(uint8_t i = 0;i<Mftp;i++){
		OnOffHelper onOffHelper ("ns3::UdpSocketFactory", InetSocketAddress (allIPs.GetAddress(4*i+1), cbrPort));

		double startTimeCBR=0;
		startTimeCBR = 1.0000+  (double) i/100.0;  
	    ...
	    cbrApps.Add (onOffHelper.Install (nodes.Get (4*i)));
	}
...
// Install application:  FTP data stream on remaining half
  ...
  uint16_t ftpPort = 54321;
   ...
   int senderWindowSize = 3000; //in bytes. This determines the sender window size used. 
   Config::SetDefault ("ns3::TcpSocket::RcvBufSize", UintegerValue(senderWindowSize)); 

   for(uint8_t i=0;i<Mftp;i++){
	   BulkSendHelper source ("ns3::TcpSocketFactory",InetSocketAddress (allIPs.GetAddress(4*i+3), ftpPort));
	    // Set the amount of data to send in bytes.  Zero is unlimited.
	    source.SetAttribute ("MaxBytes", UintegerValue (maxBytes));
	    ApplicationContainer sourceApps = source.Install (nodes.Get (4*i+2));
		double startTimeFTP =0;
	    startTimeFTP = 1.0001+  (double) i/100.0;  
	   sourceApps.Start (Seconds (startTimeFTP));
	    ...
	    ApplicationContainer sinkApps = sinkFTP.Install (nodes.Get (4*i+2));
  ...
  std::cout << "Average channel throughput = " << totalTput/(Mftp) << "Mbps" << std::endl;
  std::cout << "FTP throughput = " << ftpTput << "Mbps" << std::endl;
  std::cout << "CBR throughput = " << (totalTput-ftpTput) << "Mbps" << std::endl;
  //std::cout << "File Transfer Delay = " << ftpDelaySum << " seconds" << std::endl;
...
int main (int argc, char **argv)
{
  ...
  for(int i=7;i<=9;i++){
	  ...
	  experiment (false, wifiManager,i);
	  std::cout << "------------------------------------------------\n";
	}
  return 0;
}
